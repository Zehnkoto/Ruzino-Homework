    ...
    if (checkPosition(layoutNode) && !_autoLayout) {
        for (UiNodePtr node : _graphNodes) {
            ...
            if (node->getNodeGraph() != nullptr) {
                ...
                if (outputCon.size() > 0) {
                    ImVec2 outputPos =
                        ed::GetNodePosition(outputCon[0]->getId());
                    ed::SetNodePosition(
                        node->getId(), ImVec2(outputPos.x - 400, outputPos.y));
...
void Graph::layoutInputs()
{
    ...
    if (_levelMap.begin() != _levelMap.end()) {
        ...
        ImVec2 startingPos =
            ed::GetNodePosition(_levelMap[levelCount].back()->getId());
        startingPos.y +=
            ed::GetNodeSize(_levelMap[levelCount].back()->getId()).y + 20;
            ...
            if (node->getNode() != nullptr) {
                ...
                ImGui::GetWindowDrawList()->AddRectFilled(
                    ...
                    ImGui::GetCursorScreenPos() +
                        ImVec2(
                            ed::GetNodeSize(node->getId()).x - 9.f,
        ...
        if (inputPin->_connected) {
            for (auto iter = _currLinks.begin(); iter != _currLinks.end();
                ...
                if (iter->_endAttr == end_attr) {
                    ...
                    // note: ed::BreakLinks doesn't work as the order ends up
                    // inaccurate
