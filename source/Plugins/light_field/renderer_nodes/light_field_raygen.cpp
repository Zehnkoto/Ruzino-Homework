...
{
	// Function content omitted	b.add_input<nvrhi::TextureHandle>("random seeds");
	b.add_input<nvrhi::TextureHandle>("random seeds");
	b.add_input<float>("Display Screen Z").min(-50).max(50).default_val(-20);
	b.add_input<float>("View Distance").min(0.1f).max(100.0f).default_val(20.0f);
	...
	b.add_input<pxr::GfVec2f>("Lens scaling").default_val({ 1,1 })
...
NODE_EXECUTION_FUNCTION(light_field_raygen)
{
	...
	auto display_screen_size = params.get_input<pxr::GfVec2f>("Display Screen Size");


	// Prepare the shader using reflection
	ProgramDesc cs_program_desc;
	cs_program_desc.shaderType = nvrhi::ShaderType::Compute;
	cs_program_desc.set_path(LIGHT_FIELD_SHADERS_DIR "/shaders/light_field_init_closest_lens.slang").set_entry_name("main");
	...
	// Create lens positions buffer (example positions - should be provided as input)
	...
	float lens_spacing = total_horizontal_spacing / 26.0f; // Divide into 27 intervals
	...
	float vertical_spacing = display_screen_size[1] / 4.0f; // Divide into 5 intervals
	...
	for (int i = 0; i < total_lenses; i++) {
		...
		lens_positions_data.push_back(GfVec3f(x * lens_scaling[0], y * lens_scaling[1], display_screen_z + lens_to_display));
	}




	auto lens_positions_buffer = create_buffer<GfVec3f>(
		params, lens_positions_data.size(), false, true);
	...
	context.begin();
	context.write_buffer(lens_positions_buffer, lens_positions_data.data(), lens_positions_data.size() * sizeof(GfVec3f));
